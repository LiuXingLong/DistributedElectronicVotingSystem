...
class RedisClient
{
	 ...
	 * @param $deriction int 0:数据入队列头(左) 1:数据入队列尾(右) 默认为1
	 * @param $repeat int 判断value是否存在  0:不判断存在 1:判断存在 如果value存在则不入队列
	 */
	public function listPush($list,$value,$direction=1,$repeat=0)
	{
		$return = null;
		switch ($direction) {
			case 0:
	 ...
	 * @param $deriction int 0:数据出队列头(左) 1:数据出队列尾(右) 默认为0
	...
	public function listInsert($list,$location=0,$value1,$value2)
	{
		...
		switch ($location) {
			case 0:
				$return = $this->_REDIS->lInsert($list, RedisClient::BEFORE, $value1, $value2);
				break;
			case 1:
				$return = $this->_REDIS->lInsert($list, RedisClient::AFTER, $value1, $value2);
				break;
			default:
				$return = false;
				break;
